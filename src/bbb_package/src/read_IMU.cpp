...
public:
    SerialNode() : Node("IMU_node"), port_("/dev/ttyUSB1"), baud_(115200 * 2), data_size_(71) {
        try {
...
private:
    ...
    void no_mag_IMU() {
        uint8_t data_mag_1[9] = {'$', 'A', 'L', 'P', 'H', 'A', ' ','0', '\n'};
        serial_->write(data_mag_1, 9U);
        RCLCPP_INFO(get_logger(), "config no mag");

        uint8_t data_mag_2[9] = {'$', 'A', 'L', 'P', 'H', 'T', ' ','0', '\n'};
    ...
    void transformation(double init_yaw, double current_yaw, double *new_yaw){
        init_yaw = deg2rad(init_yaw);
        current_yaw = deg2rad(current_yaw);
        *new_yaw = acos(cos(init_yaw)*cos(current_yaw)+sin(init_yaw)*sin(current_yaw));
    ...
    void timer_callback() {
        if (serial_->available() >= data_size_) {
            ...
            if (counter <11){
                ...
                transformation(init_yaw,yaw,&yaw);
