...
class DataProcessingNode : public rclcpp::Node {
 public:
  DataProcessingNode()
    : Node("data_processing_node_straight"), angleIMU(0.0), actual_position({0.0, 0.0}), pre_actual_position({0.0, 0.0}), index(0),pre_index (0), d_min(0.0) {
    initializeArrays();
 ...
 private:
  ...
  void timer_callback() {
    ...
    if (index == pre_index && index >200) index = index +1;
    pre_index = index;
