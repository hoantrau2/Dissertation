...
//  public:
//   DataProcessingNode()
//     : Node("Stanley_node"), angleIMU(0.0), actual_position({0.0, 0.0}), pre_actual_position({0.0, 0.0}), index(0),pre_index (0), d_min(0.0) {
//     initializeArrays();
...
//       theta[i] = std::atan2((y[i] - y[i - 1]), (x[i] - x[i - 1]));
...
#define C 2.5
#define D 3.5
#define STEP_DISTANCE 0.008
#define COUNTER 600

class DataProcessingNode : public rclcpp::Node {
 public:
  DataProcessingNode()
    : Node("Stanley_node"), angleIMU(0.0), actual_position({0.0, 0.0}), pre_actual_position({0.0, 0.0}), index(0), d_min(0.0) {
    initializeArrays();
    setupSubscribersAndPublishers();
    setupTimer();
 ...
 void timer_callback() {
    d_min = std::sqrt(std::pow((actual_position[0] - x[int(index)]), 2) + std::pow((actual_position[1] - y[int(index)]), 2));
     if (index <= 4) {
       for (int i = index + 1; i < std::min(index + 4, COUNTER); i++) {
      double d = std::sqrt(std::pow((actual_position[0] - x[i]), 2) + std::pow((actual_position[1] - y[i]), 2));
      if (d < d_min) {
        d_min = d;
        index = i;
     ...
     {
       for (int i = index -4; i < std::min(index + 4, COUNTER); i++) {
      double d = std::sqrt(std::pow((actual_position[0] - x[i]), 2) + std::pow((actual_position[1] - y[i]), 2));
      if (d < d_min) {
        d_min = d;
        index = i;
    ...
    double arctann = std::atan2(Kp * d_min, Ksoft + 0.3);
    // if (arctann > 40 * M_PI / 180) arctann = 40 * M_PI / 180;
